	...
	<head>
        ...
        <script type="text/javascript">
			function validateForm(){
				var x = document.forms["mtgForm"]["peepFName"].value;
				if (x == ""){
...
<script>
	function validateForm(){
		...
		for (var i - 0; i < params.length; i++){
			...
			if (p[0] == "Action"){
				theAction = decodeURIComponent(p[1]));
			}
		}
		switch(action){
		case "Add":
			alert("Adding a new person");
			break;
		case "Edit":
			alert("Updating existing person");
			break;
		}
		alert("We have no clue");
		return false;
	}
	function deleteUser(){
		alert("Are you sure?");
...
function showForm($action, $origin, $destination, $ID){
   ...
   if($connection->errno > 0){
    ...
    echo "<form id='mtgForm' action='" . $dest . "' method='post' onsubmit='return validateForm()'>";
    ...
    echo "<tr><td></td><td><input type='submit' value='Ok' size='10'/>&nbsp;&nbsp;";
    echo "&nbsp;&nbsp;<button type='button' id='deleteBtn' style='background:red; color:white;' onclick='DeleteUser()'>&nbsp;DELETE&nbsp;</button></td></tr>";
    echo "</table></center>";
