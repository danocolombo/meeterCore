					...
					<table id="formTable">
						<tr>
                      	...
                      	<tr>
                              ...
                              <?php 
                                if($theMeeting->getMtgType() == "Lesson"){
                              ...
                              <?php 
                                if($theMeeting->getMtgType()== "Testimony"){
                              ...
                              <?php 
                                if($theMeeting->getMtgType() == "Special"){
            			...
            			<tr>
                			...
                			<td><input id="mtgTitle" size="40" style="font-size:14;" type="text" value="<?php echo $theMeeting->getMtgTitle();?>"/></td>
            			</tr>
            			<tr>
							<td>
							...
							<select id="mtgCoordinator" name="mtgCoordinator">
    							<?php 
    								for($i = 0; $i < sizeof($hostPeeps); $i++){
    								    if($theMeeting->getHost()==$hostPeeps[$i][0]){
    							...
    							<select id="mtgWorship" valign="center" name="mtgWorship">
    								<?php 
        								...
        								for($i = 0; $i < sizeof($worshipPeeps); $i++){
        								    if($theMeeting->getWorship() == $worshipPeeps[$i][0]){
    							...
    							<select id="mtgAV" name="mtgAV">
    								<?php 
        								...
        								for($i = 0; $i < sizeof($avPeeps); $i++){
        								    if($theMeeting->getAv() == $avPeeps[$i][0]){
                    		...
                    		<tr>
    							...
    							<select id="mtgGreeter1" name="mtgGreeter1">
    								<?php 
        								...
        								for($i = 0; $i < sizeof($greeterPeeps); $i++){
        								    if($theMeeting->getGreeter1() == $greeterPeeps[$i][0]){
    							...
    							<select id="mtgGreeter2" name="mtgGreeter2">
    								<?php 
    								    //echo "<option disabled selected>Please pick one</option>";
        								for($i = 0; $i < sizeof($greeterPeeps); $i++){
        								    if($theMeeting->getGreeter2() == $greeterPeeps[$i][0]){
        								        echo "<option value=\"" . $greeterPeeps[$i][0] . "\" selected>" . $greeterPeeps[$i][1] . "</option>";
        								    }else{
        								        echo "<option value=\"" . $greeterPeeps[$i][0] . "\">" . $greeterPeeps[$i][1] . "</option>";
        								    }
    							...
    							<select id="mtgResources" name="mtgResources">
    								<?php 
        								...
        								for($i = 0; $i < sizeof($resourcePeeps); $i++){
        								    if($theMeeting->getResources() == $resourcePeeps[$i][0]){
    						...
    						<tr>
    							...
    							<select id="mtgReader1" name="mtgReader1">
    								<?php 
        								for($i = 0; $i < sizeof($readerPeeps); $i++){
        								    if($theMeeting->getReader1() == $readerPeeps[$i][0]){
        								        echo "<option value=\"" . $readerPeeps[$i][0] . "\" selected>" . $readerPeeps[$i][1] . "</option>";
        								    }else{
    							...
    							<select id="mtgReader2" name="mtgReader2">
    								<?php 
        								for($i = 0; $i < sizeof($readerPeeps); $i++){
        								    for($i = 0; $i < sizeof($readerPeeps); $i++){
        								        if($theMeeting->getReader2() == $readerPeeps[$i][0]){
        								            echo "<option value=\"" . $readerPeeps[$i][0] . "\" selected>" . $readerPeeps[$i][1] . "</option>";
        								        }else{
        								            echo "<option value=\"" . $readerPeeps[$i][0] . "\">" . $readerPeeps[$i][1] . "</option>";
        								        }
					    ...
					    ?>	
    						<tr>
    							...
    							<select id="mtgAnnouncements" name="mtgAnnouncements">
    								<?php 
    								for($i = 0; $i < sizeof($announcementPeeps); $i++){
        								    if($theMeeting->getAnnouncements() == $announcementPeeps[$i][0]){
        								        echo "<option value=\"" . $announcementPeeps[$i][0] . "\" selected>" . $announcementPeeps[$i][1] . "</option>";
        								    }else{
					    ...
					    ?>
    						...
    						<tr>
    							...
    							<select id="mtgChips1" name="mtgChip1">
    								<?php 
        								for($i = 0; $i < sizeof($chipPeeps); $i++){
        								    if($theMeeting->getChips1() == $chipPeeps[$i][0]){
        								        echo "<option value=\"" . $chipPeeps[$i][0] . "\" selected>" . $chipPeeps[$i][1] . "</option>";
        								    }else{
    							...
    							<select id="mtgChips2" name="mtgChip2">
    								<?php 
        								for($i = 0; $i < sizeof($chipPeeps); $i++){
        								    if($theMeeting->getChips2() == $chipPeeps[$i][0]){
        								        echo "<option value=\"" . $chipPeeps[$i][0] . "\" selected>" . $chipPeeps[$i][1] . "</option>";
        								    }else{
        								        echo "<option value=\"" . $chipPeeps[$i][0] . "\">" . $chipPeeps[$i][1] . "</option>";
        								    }
    						...
    						<tr>
    							...
    							<select id="mtgSerenity" name="mtgSerenity">
    								...
    								for($i = 0; $i < sizeof($serenityPeeps); $i++){
    								    if($theMeeting->getSerenity() == $serenityPeeps[$i][0]){
    								        echo "<option value=\"" . $serenityPeeps[$i][0] . "\" selected>" . $serenityPeeps[$i][1] . "</option>";
    								    }else{
    						...
    						<tr>
    							...
    							<select id="mtgCafe" name="mtgCafe">
    								...
    								for($i = 0; $i < sizeof($cafePeeps); $i++){
    								    if($theMeeting->getCafeFac()== $cafePeeps[$i][0]){
    								        echo "<option value=\"" . $cafePeeps[$i][0] . "\" selected>" . $cafePeeps[$i][1] . "</option>";
    								    }else{
						...
						<tr>
							...
							<?php 
							if(sizeof($theMeeting->getDonationAmount()) > 0){
							    echo "<td><input id=\"mtgDonations\" size=\"6\" type=\"text\" placeholder=\"$ 0\"/>" . $theMeeting->getDonationAmount() . "</td>";
							}else{
							    echo "<td><input id=\"mtgDonations\" size=\"6\" type=\"text\" placeholder=\"$ 0\"/></td>";
							}
							?>
    						...
    						<tr>
    							...
    							<select id="mtgMealFac" name="mtgMealFac">
    								...
    								for($i = 0; $i < sizeof($mealPeeps); $i++){
    								    if($theMeeting->getMealFac()== $mealPeeps[$i][0]){
    								        echo "<option value=\"" . $mealPeeps[$i][0] . "\" selected>" . $mealPeeps[$i][1] . "</option>";
    								    }else{
            							...
            							<td>&nbsp;&nbsp;Facilitator:&nbsp;<select id="mtgNurseryFac" name="mtgNurseryFac">
            								<?php 
                								for($i = 0; $i < sizeof($nurseryPeeps); $i++){
                								    if($theMeeting->getNurseryFac()== $nurseryPeeps[$i][0]){
                								        echo "<option value=\"" . $nurseryPeeps[$i][0] . "\" selected>" . $nurseryPeeps[$i][1] . "</option>";
                								    }else{
            							...
            							<td>&nbsp;&nbsp;Facilitator:&nbsp;<select id="mtgChildrensFac" name="mtgChilrensFac">
            								<?php 
                								for($i = 0; $i < sizeof($childrenPeeps); $i++){
                								    if($theMeeting->getChildrenFac()== $childrenPeeps[$i][0]){
                								        echo "<option value=\"" . $childrenPeeps[$i][0] . "\" selected>" . $childrenPeeps[$i][1] . "</option>";
                								    }else{
            							...
            							<td>&nbsp;&nbsp;Facilitator:&nbsp;<select id="mtgYouthFac" name="mtgYouthFac">
            								<?php 
                								for($i = 0; $i < sizeof($youthPeeps); $i++){
                								    if($theMeeting->getYouthFac()== $youthPeeps[$i][0]){
                								        echo "<option value=\"" . $youthPeeps[$i][0] . "\" selected>" . $youthPeeps[$i][1] . "</option>";
                								    }else{
    						...
    						<tr>
    							...
    							<select id="mtgSetup" name="mtgSetup">
    								...
    								for($i = 0; $i < sizeof($setupPeeps); $i++){
    								    if($theMeeting->getSetup()== $setupPeeps[$i][0]){
    								        echo "<option value=\"" . $setupPeeps[$i][0] . "\" selected>" . $setupPeeps[$i][1] . "</option>";
    								    }else{
    						...
    						<tr>
    							...
    							<select id="mtgTearDown" name="mtgTearDown">
    								...
    								for($i = 0; $i < sizeof($tearDownPeeps); $i++){
    								    if($theMeeting->teardown== $tearDownPeeps[$i][0]){
    								        echo "<option value=\"" . $tearDownPeeps[$i][0] . "\" selected>" . $tearDownPeeps[$i][1] . "</option>";
    								    }else{
    							...
    							<a href="#" title="People on Clean-up team"><img style="width:15px;height:15px;" src="images/toolTipQM.png"/></a>
        						</td>
    						</tr>
						<?php
    						...
    						<tr>
    							...
    							<select id="mtgTransportation" name="mtgTransportation">
    								...
    								for($i = 0; $i < sizeof($transportationPeeps); $i++){
    								    if($theMeeting->getTransportation()== $transportationPeeps[$i][0]){
    								        echo "<option value=\"" . $transportationPeeps[$i][0] . "\" selected>" . $transportationPeeps[$i][1] . "</option>";
    								    }else{
    							...
    							<a href="#" title="People on Transportation team"><img style="width:15px;height:15px;" src="images/toolTipQM.png"/></a>
        						</td>
    						</tr>
						<?php
    						...
    						<tr>
    							...
    							<select id="mtgSecurity" name="mtgSecurity">
    								...
    								for($i = 0; $i < sizeof($securityPeeps); $i++){
    								    if($theMeeting->getSecurity()== $securityPeeps[$i][0]){
    								        echo "<option value=\"" . $securityPeeps[$i][0] . "\" selected>" . $securityPeeps[$i][1] . "</option>";
    								    }else{
						...
						<tr><td colspan="2">
							<fieldset>
                              	...
                              	<?php 
                              	if(sizeof($theMeeting->getNotes()) > 0){
                              	     echo "<textarea id=\"mtgNotes\" rows=\"5\" cols=\"50\">" . $theMeeting->getNotes() . "</textarea>";
                              	}else{
                              	    echo "<textarea id=\"mtgNotes\" rows=\"5\" cols=\"50\"></textarea>";
                              	}
                              	?>
                        	</fieldset>
                            </td></tr>	
